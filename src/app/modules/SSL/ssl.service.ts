...
const initPayment = async (paymentData: TPaymentData) => {
    try {
        const data = {
            ...
            success_url: `${config.ssl.ssl_base_url}/payment?status=success`,
            fail_url: `${config.ssl.ssl_base_url}/payment?status=failed`,
            cancel_url: `${config.ssl.ssl_base_url}/payment?status=cancel`,
